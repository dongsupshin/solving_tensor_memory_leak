#!/usr/bin/env python3
"""
fourth_try/app.py — clear_session + model-rebuild memory trend test profile.
...
Live RSS trend: FastAPI + Chart.js on ``FOURTH_TRY_WEB_HOST`` / ``FOURTH_TRY_WEB_PORT`` (defaults ``0.0.0.0:8765``), same pattern as ``second_try``.
...
WEB_HOST = os.environ.get("FOURTH_TRY_WEB_HOST", "0.0.0.0")
WEB_PORT = int(os.environ.get("FOURTH_TRY_WEB_PORT", "8765"))
...
def _inference_loop(cfg: RunConfig) -> None:
    ...
    try:
        ...
        print("[fourth_try] Inference loop (clear_session + rebuild)")
        print(
        ...
        print(f"  rebuild_after_clear = True   (model reloaded after clear)")
        ...
        while not _stop_event.is_set():
            ...
            # Rebuild model after every clear to compare RSS trend vs third_try behavior.
...
<head>
  ...
  <title>RSS trend — fourth_try (clear+rebuild)</title>
...
<body>
  <header>
    <h1>RSS (resident set) — fourth_try clear+rebuild loop</h1>
...
def main() -> None:
    ...
    p = argparse.ArgumentParser(
        description="fourth_try clear+rebuild leak test + RSS web chart. "
    ...
    print(
        f"[fourth_try] Web UI http://127.0.0.1:{WEB_PORT}/  (bind {WEB_HOST}:{WEB_PORT})",
        flush=True,
    )
