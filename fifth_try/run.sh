#!/usr/bin/env bash
# fifth_try: #58676 diagnosis harness (Python vs TF/native memory) + stress flags.
# Default: MALLOC_ARENA_MAX=2 (glibc malloc arena cap, same idea as second_try). Override: MALLOC_ARENA_MAX=8 ./run.sh
#
#   chmod +x run.sh && ./run.sh
# Heavier load: ./run.sh -- --opt1 --opt2 --opt3
# Engine compare: ./run.sh -- --opt3 --engine predict   (keras predict_on_batch path)
#                 ./run.sh -- --opt3 --engine tf_fn     (single traced tf.function path, default)
set -euo pipefail
: "${MALLOC_ARENA_MAX:=2}"
